...
$tab3= mysql_fetch_assoc(database::query("select * from $messages  where uid_fk='".$_SESSION["uid"]."' ORDER BY `msg_id` DESC LIMIT 1"));
	if($_GET['send']=='1'&&(strtotime($dat1))-strtotime($tab3['created'])<$_SESSION["unsend"])
	{
			database::query("DELETE FROM $messages Where msg_id='".$tab3['msg_id']."'");
			//$_GET['send']=0;
	}
	//else{echo '<script>alert("Sorry, you have exceeded the timelimit. The message cant be unsent.");</script>';}
?>
	
		<a href="post.php?tid=<? echo $messages ?>&send=<? if($_GET['send']==1) echo '0'; else echo '1'; ?>" class="button postfb" id="textpost" title=Unsend align="right" style="float:right;">
<?
		if($_GET['send']=='1'||((strtotime($dat1))-strtotime($tab3['created']))>$_SESSION["unsend"])
		{echo '';}
		else
		{//echo ((strtotime($dat1))-strtotime($tab3['created']));
		echo 'Unsend';}
?>
		</a>
<br>
...
<!--- <div id="viewpost"> --->
	...
	$qu=$db->fetch($quw);
	$i=0;
	$tab8= mysql_fetch_assoc(database::query("select * from $messages  where uid_fk='".$_SESSION["uid"]."' ORDER BY `msg_id` DESC LIMIT 1"));
		if( $tab8['seen']!=NULL)
		{		
			echo '<div><font size=1 color="#BDBCDB" style="float:right;">&#10004; seen on '.date('F j, g:i:s a',(strtotime($tab8['seen']))+19800).'</font></div><br>';
		}
	...
	if(isset($_POST["msgid"]))
		{
	$id=$_POST["msgid"];
	//database::query("DELETE from $messages where msg_id='".$id."'");
    ...
    <div id="display">    
		...
		<div align="right">	<input type="submit" name="msgid" value="<? echo $query["msg_id"]; ?> " title="delete">
		</div>
		</form>
